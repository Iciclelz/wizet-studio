...
namespace Studio.Wizet.Library.Wz.Serialization
{
    ...
    public class WzPngMp3Serializer : ProgressingWzSerializer, IWzImageSerializer, IWzObjectSerializer
    {
        ...
        public void SerializeObject(IWzObject obj, string outPath)
        {
            ...
            {
                outPath = string.Concat(outPath, "\\");
            }
            this.total = this.CalculateTotal(obj);
            this.ExportRecursion(obj, outPath);
        }
        ...
        private void ExportRecursion(IWzObject currObj, string outPath)
        {
            ...
            if (currObj != null)
            {
                currObjName = Path.GetFileName(currObj.Name);
            }
            if (currObj is WzFile)
            {
            ...
            {
                ((WzCanvasProperty)currObj).PngProperty.GetPNG(false).Save(outPath + currObjName + ".png", ImageFormat.Png);
            }
            else if (currObj is WzSoundProperty)
            {
                ((WzSoundProperty)currObj).SaveToFile(outPath + currObjName + ".mp3");
            }
