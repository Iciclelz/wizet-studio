...
namespace Studio.Wizet.Hexview
{
    ...
    /// </summary>
    [ToolboxBitmap(typeof(ByteEditor), "ByteEditor.bmp")]
	public class ByteEditor : Control
	{
		...
		class EmptyKeyInterpreter : IKeyInterpreter
		{
			...
			public EmptyKeyInterpreter(ByteEditor ByteEditor)
			{
			...
			{ return _ByteEditor.BasePreProcessMessage(ref m); }
		...
		class KeyInterpreter : IKeyInterpreter
		{
			...
			#region Fields
			/// <summary>
			/// Contains the parent ByteEditor control
			/// </summary>
			protected ByteEditor _ByteEditor;
			...
			#region Ctors
			public KeyInterpreter(ByteEditor ByteEditor)
			{
				_ByteEditor = ByteEditor;
			}
			#endregion
			...
			{
				_ByteEditor.MouseDown += new MouseEventHandler(BeginMouseSelection);
				_ByteEditor.MouseMove += new MouseEventHandler(UpdateMouseSelection);
				_ByteEditor.MouseUp += new MouseEventHandler(EndMouseSelection);
			}
			...
			void BeginMouseSelection(object sender, MouseEventArgs e)
			{
				...
				if (!_shiftDown)
				{
					_bpiStart = new BytePositionInfo(_ByteEditor._bytePos, _ByteEditor._byteCharacterPos);
					_ByteEditor.ReleaseSelection();
				}
				else
				{
			...
			void UpdateMouseSelection(object sender, MouseEventArgs e)
			{
				...
				{
					realselStart = _ByteEditor._bytePos;
					realselLength = 0;
				}

				if (realselStart != _ByteEditor._bytePos || realselLength != _ByteEditor._selectionLength)
				{
					_ByteEditor.InternalSelect(realselStart, realselLength);
					_ByteEditor.ScrollByteIntoView(_bpi.Index);
				}
			}
			...
			{
				_ByteEditor.ScrollByteIntoView();
				return _ByteEditor.BasePreProcessMessage(ref m);
			}
			...
			{
				long pos = _ByteEditor._bytePos;
				int cp = _ByteEditor._byteCharacterPos;

				if (!(pos == 0 && cp == 0))
				{
					pos = Math.Max(-1, pos - _ByteEditor._iHexMaxHBytes);
					if (pos == -1)
						return true;

					_ByteEditor.SetPosition(pos);

					if (pos < _ByteEditor._startByte)
					{
						_ByteEditor.PerformScrollLineUp();
					}

					_ByteEditor.UpdateCaret();
					_ByteEditor.Invalidate();
				}
			...
			protected virtual bool PreProcessWmKeyDown_Down(ref Message m)
			{
				...
				if (pos == _ByteEditor._byteProvider.Length && cp == 0)
					return true;

				pos = Math.Min(_ByteEditor._byteProvider.Length, pos + _ByteEditor._iHexMaxHBytes);

				if (pos == _ByteEditor._byteProvider.Length)
					cp = 0;

				_ByteEditor.SetPosition(pos, cp);

				if (pos > _ByteEditor._endByte - 1)
				{
					_ByteEditor.PerformScrollLineDown();
				}
			...
			protected virtual bool PreProcessWmKeyDown_PageUp(ref Message m)
			{
				...
				pos = Math.Max(0, pos - _ByteEditor._iHexMaxBytes);
				if (pos == 0)
					return true;
				...
				if (pos < _ByteEditor._startByte)
				{
					_ByteEditor.PerformScrollPageUp();
				}
			...
			protected virtual bool PreProcessWmKeyDown_PageDown(ref Message m)
			{
				...
				pos = Math.Min(_ByteEditor._byteProvider.Length, pos + _ByteEditor._iHexMaxBytes);
				...
				if (pos > _ByteEditor._endByte - 1)
				{
					_ByteEditor.PerformScrollPageDown();
				}
			...
			{
				long pos = _ByteEditor._bytePos;
				long sel = _ByteEditor._selectionLength;
				...
				_ByteEditor.ScrollByteIntoView();
				_ByteEditor.InternalSelect(pos, sel);
			...
			protected virtual bool PreProcessWmKeyDown_ShiftUp(ref Message m)
			{
				...
				if (pos - _ByteEditor._iHexMaxHBytes < 0 && pos <= _bpiStart.Index)
					return true;

				if (_bpiStart.Index >= pos + sel)
				{
					pos = pos - _ByteEditor._iHexMaxHBytes;
					sel += _ByteEditor._iHexMaxHBytes;
					_ByteEditor.InternalSelect(pos, sel);
					_ByteEditor.ScrollByteIntoView();
				}
				else
				{
					...
					else
					{
						sel -= _ByteEditor._iHexMaxHBytes;
						_ByteEditor.InternalSelect(pos, sel);
						_ByteEditor.ScrollByteIntoView(pos + sel);
					}
				}
			...
			protected virtual bool PreProcessWmKeyDown_ShiftRight(ref Message m)
			{
				...
				if (pos + sel >= _ByteEditor._byteProvider.Length)
					return true;
			...
			protected virtual bool PreProcessWmKeyDown_ShiftDown(ref Message m)
			{
				...
				long max = _ByteEditor._byteProvider.Length;

				if (pos + sel + _ByteEditor._iHexMaxHBytes > max)
					return true;
				...
				else
				{
					...
					{
						pos += _ByteEditor._iHexMaxHBytes;
						//sel -= _ByteEditor._iHexMaxHBytes;
					}
			...
			{
				if (_ByteEditor._stringViewVisible && _ByteEditor._keyInterpreter.GetType() == typeof(KeyInterpreter))
				{
					_ByteEditor.ActivateStringKeyInterpreter();
					_ByteEditor.ScrollByteIntoView();
					_ByteEditor.ReleaseSelection();
					_ByteEditor.UpdateCaret();
					_ByteEditor.Invalidate();
				...
				if (_ByteEditor.Parent == null) return true;
				_ByteEditor.Parent.SelectNextControl(_ByteEditor, true, true, true, true);
				return true;
			}

			protected virtual bool PreProcessWmKeyDown_ShiftTab(ref Message m)
			{
				if (_ByteEditor._keyInterpreter is StringKeyInterpreter)
				{
					_shiftDown = false;
					_ByteEditor.ActivateKeyInterpreter();
					_ByteEditor.ScrollByteIntoView();
					_ByteEditor.ReleaseSelection();
					_ByteEditor.UpdateCaret();
					_ByteEditor.Invalidate();
				...
				if (_ByteEditor.Parent == null) return true;
				_ByteEditor.Parent.SelectNextControl(_ByteEditor, false, true, true, true);
				return true;
			}

			protected virtual bool PreProcessWmKeyDown_Back(ref Message m)
			{
				if (!_ByteEditor._byteProvider.SupportsDeleteBytes())
					return true;

				if (_ByteEditor.ReadOnly)
					return true;
				...
				_ByteEditor._byteProvider.DeleteBytes(Math.Max(0, startDelete), bytesToDelete);
				_ByteEditor.UpdateScrollSize();
			...
			protected virtual bool PreProcessWmKeyDown_Delete(ref Message m)
			{
				...
				long bytesToDelete = (sel > 0) ? sel : 1;
				_ByteEditor._byteProvider.DeleteBytes(pos, bytesToDelete);
			...
			protected virtual bool PreProcessWmKeyDown_End(ref Message m)
			{
				...
				if (pos >= _ByteEditor._byteProvider.Length - 1)
					return true;

				pos = _ByteEditor._byteProvider.Length;
				cp = 0;
				_ByteEditor.SetPosition(pos, cp);
			...
			protected virtual bool PreProcessWmKeyDown_ShiftShiftKey(ref Message m)
			{
				...
				if (_ByteEditor._selectionLength > 0)
					return true;
			...
			public virtual bool PreProcessWmChar(ref Message m)
			{
				...
				bool sw = _ByteEditor._byteProvider.SupportsWriteByte();
				bool si = _ByteEditor._byteProvider.SupportsInsertBytes();
				bool sd = _ByteEditor._byteProvider.SupportsDeleteBytes();
				...
				if (
					(!sw && pos != _ByteEditor._byteProvider.Length) ||
					(!si && pos == _ByteEditor._byteProvider.Length))
				{
					return _ByteEditor.BasePreProcessMessage(ref m);
				}
				...
				if (Uri.IsHexDigit(c))
				{
					...
					bool isInsertMode = (pos == _ByteEditor._byteProvider.Length);

					// do insert when insertActive = true
					if (!isInsertMode && si && _ByteEditor.InsertActive && cp == 0)
						isInsertMode = true;

					if (sd && si && sel > 0)
					{
						_ByteEditor._byteProvider.DeleteBytes(pos, sel);
						isInsertMode = true;
						cp = 0;
						_ByteEditor.SetPosition(pos, cp);
					}
					...
					if (isInsertMode)
						...
						currentByte = _ByteEditor._byteProvider.ReadByte(pos);
					...
					if (isInsertMode)
						_ByteEditor._byteProvider.InsertBytes(pos, new byte[] { newcb });
					else
						_ByteEditor._byteProvider.WriteByte(pos, newcb);
			...
			{
				_ByteEditor.InsertActive = !_ByteEditor.InsertActive;
				return true;
			}
			...
			protected virtual bool PerformPosMoveRight()
			{
				...
				else
				{
					if (!(pos == _ByteEditor._byteProvider.Length && cp == 0))
					{
						...
						if (cp > 0)
						{
							pos = Math.Min(_ByteEditor._byteProvider.Length, pos + 1);
							cp = 0;
						}
						else
						{
			...
			public virtual PointF GetCaretPointF(long byteIndex)
			{
				...
				return _ByteEditor.GetBytePointF(byteIndex);
			}

			protected virtual BytePositionInfo GetBytePositionInfo(Point p)
			{
				return _ByteEditor.GetHexBytePositionInfo(p);
			}
			#endregion
		}
		#endregion
		...
		{
			#region Ctors
			public StringKeyInterpreter(ByteEditor ByteEditor)
				: base(ByteEditor)
			{
				_ByteEditor._byteCharacterPos = 0;
			}
			#endregion
			...
			public override bool PreProcessWmChar(ref Message m)
			{
				...
				// do insert when insertActive = true
				if (!isInsertMode && si && _ByteEditor.InsertActive)
					isInsertMode = true;
				...
				byte b = _ByteEditor.ByteCharConverter.ToByte(c);
				if (isInsertMode)
					_ByteEditor._byteProvider.InsertBytes(pos, new byte[] { b });
				else
					_ByteEditor._byteProvider.WriteByte(pos, b);
			...
			public override PointF GetCaretPointF(long byteIndex)
			{
				...
				Point gp = _ByteEditor.GetGridBytePoint(byteIndex);
				return _ByteEditor.GetByteStringPointF(gp);
			}

			protected override BytePositionInfo GetBytePositionInfo(Point p)
			{
				return _ByteEditor.GetStringBytePositionInfo(p);
			}
			#endregion
		}
		#endregion
		...
		/// <summary>
		/// Initializes a new instance of a ByteEditor class.
		/// </summary>
		public ByteEditor()
		{
		...
		{
			System.Diagnostics.Debug.WriteLine("UpdateScrollSize()", "ByteEditor");
		...
		void UpdateVScroll()
		{
			System.Diagnostics.Debug.WriteLine("UpdateVScroll()", "ByteEditor");
		...
		{
			System.Diagnostics.Debug.WriteLine("ScrollByteIntoView()", "ByteEditor");
		...
		{
			System.Diagnostics.Debug.WriteLine("ScrollByteIntoView(long index)", "ByteEditor");
		...
		{
			System.Diagnostics.Debug.WriteLine("ReleaseSelection()", "ByteEditor");
		...
		void CreateCaret()
		{
			...
			System.Diagnostics.Debug.WriteLine("CreateCaret()", "ByteEditor");
		...
		void UpdateCaret()
		{
			...
			System.Diagnostics.Debug.WriteLine("UpdateCaret()", "ByteEditor");
		...
		void DestroyCaret()
		{
			...
			System.Diagnostics.Debug.WriteLine("DestroyCaret()", "ByteEditor");
		...
		{
			System.Diagnostics.Debug.WriteLine("SetCaretPosition()", "ByteEditor");
		...
		{
			System.Diagnostics.Debug.WriteLine("GetHexBytePositionInfo()", "ByteEditor");
		...
		{
			System.Diagnostics.Debug.WriteLine("GetStringBytePositionInfo()", "ByteEditor");
		...
		protected override void OnPaint(PaintEventArgs e)
		{
			...
			System.Diagnostics.Debug.WriteLine("OnPaint " + DateTime.Now.ToString(), "ByteEditor");
		...
		/// <summary>
		/// The font used to display text in the ByteEditor.
		/// </summary>
		...
		/// <summary>
		/// Gets or sets whether the ByteEditor control displays the hex characters in upper or lower case.
		/// </summary>
		[DefaultValue(typeof(HexCasing), "Upper"), Category("Hex"), Description("Gets or sets whether the ByteEditor control displays the hex characters in upper or lower case.")]
		...
		{
			System.Diagnostics.Debug.WriteLine("OnMouseDown()", "ByteEditor");
		...
		{
			System.Diagnostics.Debug.WriteLine("OnGotFocus()", "ByteEditor");
		...
		{
			System.Diagnostics.Debug.WriteLine("OnLostFocus()", "ByteEditor");
